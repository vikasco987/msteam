...
//   //   setIsPanelOpen(true);
//   //   setShowPaymentHistory(false);
//   //     setSelectedTask(null); // Reset previous task to prevent overlap
...
// // Loading state for initial fetch & subsequent updates


// // When a task is selected, update the input amounts
// useEffect(() => {
//   if (selectedTask) {
//     const latestPayment = selectedTask.paymentHistory?.[selectedTask.paymentHistory.length - 1];
//     setCurrentAmountInput(latestPayment?.amount?.toString() || selectedTask.amount?.toString() || "");
...
// // Fetch tasks for pagination or updates
// const fetchTasks = useCallback(
//   async (page: number = currentPage, limit: number = tasksPerPage) => {
//     setLoading(true);
//     try {
//       const token = await getToken();
//       const res = await fetch(`/api/timeline?page=${page}&limit=${limit}`, {
...
//       if (!res.ok) {
//         const errorData = await res.json();
//         console.error(`Failed to fetch tasks: ${res.status}`, errorData);
//         setTasks([]);
//         setTotalPages(1);
//         setSelectedTask(null);
//         setSelectedTaskId(null);
...
// // Trigger fetch whenever page or limit changes
// useEffect(() => {
//   fetchTasks(currentPage, tasksPerPage);
// }, [currentPage, tasksPerPage, fetchTasks]);

// // Fetch assignee details when tasks update
// useEffect(() => {
...
// // Compute all unique assignees
// const allAssignees = useMemo(() => Array.from(new Set(tasks.flatMap(t => t.assigneeIds || []))), [tasks]);

// // Filter tasks by search or assignee
// const filteredTasks = useMemo(() => {
//   let currentTasks = [...tasks];
//   if (searchTerm) {
...
//       task.shop.toLowerCase().includes(term) ||
//       task.customer.toLowerCase().includes(term)
//     );
//   }
//   if (selectedAvatar) {
//     currentTasks = currentTasks.filter(task => (task.assigneeIds || []).includes(selectedAvatar));
//   }
//   return currentTasks;
// }, [tasks, searchTerm, selectedAvatar, tasksVersion]);

// // When a task is clicked, open panel after resetting previous
// const handleTaskClick = useCallback(async (task: Task) => {
//   setSelectedTaskId(task.id);
//   setIsPanelOpen(true);
//   setShowPaymentHistory(false);
//   setSelectedTask(null); // Prevent overlap while loading new task
