...
export async function GET(req: Request) {
  try {
    ...
    let list = Object.values(byEmployee).map((emp: any) => {
      // ⚡ Updated fair scoring formula
      const presenceScore = emp.daysPresent >= 8 ? 10 : (emp.daysPresent / 8) * 10; // reward full presence only for 8+ days
      const workingHoursScore = emp.totalWorkingHours * 1; // 1 point per hour
      const overtimeScore = emp.overtimeHours * 1; // 1 point per overtime hour
      const earlyArrivalScore = emp.earlyArrival * 1; // smaller bonus
      const latePenalty = emp.daysLate * 1.5; // smaller penalty
      const earlyLeavePenalty = emp.earlyLeaves * 1.5;
    ...
    // Sort by score → then by total working hours → then by days present
    list.sort((a: any, b: any) => {
      if (b.score !== a.score) return b.score - a.score;
