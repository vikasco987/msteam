...
//     const {
//       phone, email, shopName, outletName, location, accountNumber, ifscCode,
//       customerName, restId, packageAmount, startDate, endDate, timeline,
//       amount, amountReceived, fields = [], aadhaarUrl, panUrl,
//       selfieUrl, chequeUrl, menuCardUrls
...
//         title: body.title,
//         status,
//         assigneeIds: Array.isArray(body.assigneeIds) ? body.assigneeIds : [body.assigneeId],
//         assignerEmail,
//         assignerName,
//         createdByClerkId: userId,
//         createdAt: new Date(),
...
// // ✅ GET with Pagination (Performance Fix)
// export async function GET(req: NextRequest) {
//   try {
//     const { userId } = await getAuth(req);
//     if (!userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
...
// // Interfaces for Clerk metadata
// interface UserPublicMetadata { role?: string; }
// interface UserPrivateMetadata { role?: string; }

// // Field & CustomFields
// interface Field { label?: string; value?: string; files?: unknown[]; }
// interface CustomFieldsInput {
//   phone?: string;
//   email?: string;
//   shopName?: string;
...
//           label: f.label || "",
//           value: f.value || "",
//           files: Array.isArray(f.files) ? f.files.filter((url): url is string => typeof url === "string") : [],
//         }))
//       : [];
...
//     // Enrichment: assigner + assignees
//     const userIdentifiers = new Set<string>();
//     tasks.forEach(task => {
//       if (task.assignerEmail) userIdentifiers.add(task.assignerEmail);
//       task.assigneeIds?.forEach((id: string) => userIdentifiers.add(id));
//     });
